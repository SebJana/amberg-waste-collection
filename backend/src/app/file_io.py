...
def load_all_waste_data():
    ...
    if not json_files:
        raise HTTPException(
            status_code=500, detail="No waste collection schedule data files found"
        )
    ...
    for json_file in json_files:
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Waste collection schedule data file {json_file.name} not found",
            )
        except json.JSONDecodeError:
            raise HTTPException(
                status_code=500,
                detail=f"Waste collection schedule data file {json_file.name} is corrupted",
            )
...
def load_zone_data(zone_code: str):
    ...
    if zone_letter not in data or zone_number not in data[zone_letter]:
        raise ZoneNotFoundError(
            f"Zone '{zone_code}' not found in waste collection data"
        )
...
def load_street_zone_mapping():
    ...
    json_file = STREET_ZONES_DIR / "street-zones-mapping.json"
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Street zone mapping file {json_file.name} not found",
        )
    except json.JSONDecodeError:
        raise HTTPException(
            status_code=500,
